...
/* Modified for use with garbage collecting malloc.
...
static error_t
argp_default_parser (int key, const char *arg, struct argp_state *state)
{
  switch (key)
    {
    case '?':
...
static error_t
argp_version_parser (int key, const char *arg, struct argp_state *state)
{
  switch (key)
    {
    case 'V':
...
static error_t
group_parse (struct group *group, struct argp_state *state, int key, 
             const char *arg)
{
  if (group->parser)
    {
...
parser_init (struct parser *parser, const struct argp *argp,
	     int argc, const char **argv, int flags, void *input)
{
  error_t err = 0;
  struct group *group;
...
#define SLEN (szs.short_len + 1)
  ...
  parser->storage = xmalloc (GLEN + CLEN + LLEN + SLEN);
...
static error_t
parser_parse_arg (struct parser *parser, const char *val)
{
...
error_t
__argp_parse (const struct argp *argp, int argc, const char **argv, unsigned flags,
