...
make_hol (const struct argp *argp, struct hol_cluster *cluster)
{
  ...
  struct hol *hol = xmalloc (sizeof (struct hol));
  ...
  if (opts)
      ...
      for (o = opts; ! oend (o); o++)
	{
      ...
      hol->entries = xmalloc (sizeof (struct hol_entry) * hol->num_entries);
      hol->short_options = xmalloc_atomic (num_short_options + 1);
...
{
  struct hol_cluster *cl = xmalloc (sizeof (struct hol_cluster));
  if (cl)
    {
      cl->group = group;
      cl->header = header;
		 ...
		 char **levels, int advance, argp_fmtstream_t stream)
{
  ...
  if (fdoc && fdoc != tdoc)
    xfree ((char *)fdoc);	/* Free user's modified doc string.  */
	  ...
	  argp_fmtstream_t stream)
{
  ...
  if (text && text != inp_text)
    xfree ((char *) text);	/* Free TEXT returned from the help filter.  */
  if (inp_text && inp_text_limit && argp->help_filter)
    xfree ((char *) inp_text);	/* We copied INP_TEXT, so free it now.  */
