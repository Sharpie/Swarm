/*
  ...
  Author: Dan Libby (dan@libby.com)
          Gennady Telegin (gepo@lvk.cs.msu.su) - add support for Objective-C
...
"#(@) $Id: xmlrpc.m,v 1.1 2007-08-30 21:14:05 gepo Exp $";
...
static XMLRPC_VALUE map_expat_errors (XML_ELEM_ERROR error) {
	...
	if (error) {
		...
		snprintf (buf, sizeof (buf),
					 ...
					 (int)error->line, (int)error->column, (int)error->byte_index);
...
XMLRPC_SERVER XMLRPC_ServerCreate () {
	...
	if (server) {
        ...
        server->objects = [Map createBegin: globalZone];
        [server->objects setCompareCStrings];
        server->objects = [server->objects createEnd];

        server->pointer2name = [Map createBegin: globalZone];
        [server->pointer2name setCompareIDs];
        server->pointer2name = [server->pointer2name createEnd];
...
int XMLRPC_ServerRegisterObject (XMLRPC_SERVER server, const char *name, id object) {
    if (server && name && object) {
        [server->objects at: (id)name insert: object];
        [server->pointer2name at: object insert: (id)name];
        return 1;
    }
...
// generate unique name for internal object
char* generateObjectName()
{
    static int counter = 0;
    char* buffer = malloc(26*sizeof(char));
    sprintf(buffer, "!internal_object_%d", counter++);
...
XMLRPC_VALUE objMethodCall(XMLRPC_SERVER server, XMLRPC_REQUEST input, void* userData)
{
    FCall_c*  fc;
    FArguments_c* fa;
    id obj = ((callParams*)userData)->target;
    char* method_name = ((callParams*)userData)->method_name;

    fa = [FArguments createBegin: getCZone(globalZone)];

    XMLRPC_VALUE xParams = XMLRPC_RequestGetData(input);   // obtain method params from request

    int n = XMLRPC_VectorSize(xParams);
    int i;

    for (i = 0; i < n; i++) {
        XMLRPC_VALUE xArg;
        if (i > 0) {
            xArg = XMLRPC_VectorNext(xParams); // obtain next parameter
        } else {
            xArg = XMLRPC_VectorRewind(xParams);
        }

        XMLRPC_VALUE_TYPE xType = XMLRPC_GetValueType(xArg);

        switch (xType) {
            case xmlrpc_int:
                {int t = XMLRPC_GetValueInt(xArg);

                [fa addInt: t];
                }
                break;
            case xmlrpc_double:
                {double t = XMLRPC_GetValueDouble(xArg);
                [fa addDouble: t];
                }
                break;
            case xmlrpc_boolean:
                {BOOL t = XMLRPC_GetValueBoolean(xArg);
                [fa addBoolean: t];
                }
                break;
            case xmlrpc_string:
                {const char* t = XMLRPC_GetValueString(xArg);
                [fa addString: t];
                }
                break;
	        case xmlrpc_vector:
	            {XMLRPC_VALUE xObj = XMLRPC_VectorRewind(xArg);

                if (XMLRPC_GetValueType(xObj) != xmlrpc_string) {
                    raiseEvent(WarningMessage, "Incorrect object definition in argument");
                }
		  
                if (strcmp(XMLRPC_GetValueString(xObj), "[!]")) {
                    raiseEvent(WarningMessage, "Incorrect object definition in argument");
	            }
                xObj = XMLRPC_VectorNext(xArg);

                const char* objName = XMLRPC_GetValueString(xObj);
                id obj = [server->objects at: (id)objName];

                if (NULL == obj) {
                    raiseEvent(WarningMessage, "Unknown server object in argument");
                }
    ...
    fc = [FCall createBegin: getCZone(globalZone)];
    [fc setMethodFromName: method_name inObject: obj];
    
    NSArgumentInfo info;
    char* type = (char*)sel_get_type((SEL)(((FCall_c*)fc)->fmethod));
      
    if (!type)
      {
        SEL aSel = sel_get_any_typed_uid (sel_get_name ((SEL)(((FCall_c*)fc)->fmethod)));
        type = (char*)sel_get_type (aSel);
      }
...
//        mframe_get_arg (argFrame, &info, &val);
        [fa setObjCReturnType: (char)*info.type];
      }
...
//              free(obj_name);
            ...
            case fcall_type_void:
                return XMLRPC_CreateValueEmpty();
            case fcall_type_boolean:
                return XMLRPC_CreateValueBoolean(NULL, ret.val.boolean);
            case fcall_type_sint:
            case fcall_type_uint:
                return XMLRPC_CreateValueInt(NULL, ret.val.sint);
            case fcall_type_double:
                return XMLRPC_CreateValueDouble(NULL, ret.val._double);
            case fcall_type_string:
                return XMLRPC_CreateValueString(NULL, ret.val.string, 0);
            case fcall_type_object:
              {
                char* name = NULL;
                // register internal object for object in result
                XMLRPC_VALUE result = XMLRPC_CreateVector(NULL, xmlrpc_vector_array);

                // marker of object result
                XMLRPC_VectorAppendString(result, NULL, "[!]", 0);

                if ([server->pointer2name containsKey: ret.val.object]) {
                    name = (char*)[server->pointer2name at: ret.val.object];
                } else {
                    name = generateObjectName();
                    XMLRPC_ServerRegisterObject(server, name, ret.val.object);
                }
                // name of object
                XMLRPC_VectorAppendString(result, NULL, name, 0);

                // protocol of communication with object
//                struct objc_protocol_list* protocol_list = ((struct objc_class*)ret.val.object)->class_pointer->protocols;

//                struct objc_protocol_list* protocol_list = objc_lookup_class([ret.val.object getName])->protocols;
                struct objc_protocol_list* protocol_list = objc_lookup_class(ret.val.object->class_pointer->name)->protocols;
                char* protocol_name;

                if (protocol_list && protocol_list->count > 0) {
                    protocol_name = (char*)[protocol_list->list[0] name];
                } else
                    raiseEvent(WarningMessage, "Unknown protocol of object");

                XMLRPC_VectorAppendString(result, NULL, protocol_name, 0);

//                free(obj_name);
                return result;
              }
            default:
                // !!!ERROR not supported by XML-RPC
                break;
            }
            return NULL;
        }
...
XMLRPC_Callback XMLRPC_ServerFindMethod (XMLRPC_SERVER server, const char *callName, void** params) {
	if (server && callName) {
        char* methodName = strchr(callName, '.');
        
        if (methodName != NULL) { // call method of object
            // get object's name
            int obj_name_length = methodName - callName;
            char* obj_name = malloc(obj_name_length + 1);
            strncpy(obj_name, callName, obj_name_length);
            obj_name[obj_name_length] = '\0';

            if ([server->objects containsKey: (id)obj_name]) {
                id obj = [server->objects at: (id)obj_name];
                free(obj_name);
                methodName++;

                *params = (void*)malloc(sizeof(callParams));
                
                ((callParams*)(*params))->target = obj;
                ((callParams*)(*params))->method_name = methodName;

                return objMethodCall;
            } else
                raiseEvent(WarningMessage, "Unknown object: %s", obj_name);
//            free(obj_name);
        }
...
XMLRPC_VALUE XMLRPC_ServerCallMethod (XMLRPC_SERVER server, XMLRPC_REQUEST request, void* userData) {
	...
	if (request && request->error) {
        ...
        arglist_t params = NULL;

		XMLRPC_Callback cb =
		XMLRPC_ServerFindMethod (server, request->methodName.str, (void**)&params);
		if (cb) {
            if (params != NULL)
                xReturn = cb (server, request, params);  // objc call
            else
