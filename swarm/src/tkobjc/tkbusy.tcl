...
namespace eval TkBusy {
  namespace export tkbusy
  variable TkBusy
  variable cursor [expr { ($tcl_platform(platform) == "unix") ? "top_left_arrow" : "arrow" }]
    ...
    if { "$cmd" == "isbusy" } {
      ...
      if { "$but" != "" } {
	append script "bindtags $but \"[bindtags $but]\";$but co -cu $cursor;bind .tkbusy <Control-c> \"$but in\";"
	...
	bind TkBusy <Control-c> { }
      ...
      set w [info comm .*]
      foreach c $w {
	append TkBusy "$c con -cu \"[$c cg -cu]\"; bindtags $c \"[bindtags $c]\";"
