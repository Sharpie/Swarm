...
#import "include.xgens.creating.m"
...
- setA: (unsigned)A setV: (unsigned)v setW: (unsigned)w
{
  ...
  maxGen = (unsigned long)LONG_MAX / sizeof (struct vGenStruct) - 1;
  ...
  if ((A > maxGen) || (v > 63) || (w > 63))
    {
      printf("Generator parameters: A = %d  v = %d  w = %d maxA = %lu  maxv = 63  maxw = 63\n\n", 
             A, v, w, maxGen);
      [InvalidCombination
        raiseEvent:
  ...
  if ((log2A+v+w) > 120) {
    printf("Generator parameters: log2(A) = %d  v = %d  w = %d maxv = 63  maxw = 63 maxSum = 120\n\n",
           log2A, v, w);
...
#import "include.xgens.setting.m"
...
- (void)describe: outStream
{
  ...
  for (j=0; j<lengthOfSeedVector; j++) {
    (void) sprintf (buffer,"     maxSeeds[%02u] = %21u\n",
                    j, maxSeedValues[j]);
    [outStream catC: buffer];
  }
  [outStream catC: "\n"];
  ...
  for (j = 0; j < lengthOfSeedVector; j++)
    {
      (void)sprintf (buffer," initialSeeds[%02u] = %21u\n",
                    j, initialSeeds[j]);
      [outStream catC: buffer];
    }
  [outStream catC: "\n"];
  ...
  for (i = 0; i < numGenerators; i++)
    {
      (void)sprintf(buffer,
                    "vGen # %03u: currSeg=%20llu currCount=%20llu\n", 
                    i,
                    vGenArr[i].currentSegment,
                    vGenArr[i].currentCount);
      [outStream catC: buffer];
    }
  [outStream catC: "\n\n"];
