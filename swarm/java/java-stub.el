  ...
  (let ((type (second argument))
    ...
    (if jni-type
        (cond ((string= type "SEL")
               (insert "SD_JAVA_ENSUREOBJCMETHOD (env, ")
               (insert argname)
               (insert ")"))
              ...
              ((string= jni-type "jclass")
               (insert "SD_JAVA_ENSUREOBJCCLASS (env, ")
               (insert argname)
               (insert ")"))
              ((string= type "JOBJECT")
               (insert argname))
              ((string= jni-type "jobject")
               (insert "SD_JAVA_ENSUREOBJC (env, ")
               (insert argname)
               (insert ")"))
...
(defun java-print-method-invocation-arguments (protocol method)
  (unless (method-factory-flag method)
    ...
    (insert " = SD_JAVA_ENSUREOBJC (env, jobj);\n"))
  (let ((arguments (method-arguments method))
        (string-pos 0)
        (module (protocol-module protocol)))
    ...
    (let* ((arguments (method-arguments method))
      ...
      (when strings
        (let ((rstrings (reverse strings)))
          (insert "  const char *strings[] = { SD_JAVA_COPYSTRING (env, ")
          (insert (first rstrings))
          (insert ")")
          (loop for string in (cdr rstrings)
                do
                (insert ",\n")
                (insert "                        SD_JAVA_COPYSTRING (env, ")
                (insert string)
                (insert ")"))
          (if (cdr rstrings) (insert "\n  ") (insert " "))
          (insert "};\n")))
      (when (convenience-create-method-p protocol method)
        (insert "  jobject nextPhase = SD_JAVA_NEXTJAVAPHASE (env, jobj);\n"))
      ...
      (let* ((signature (get-method-signature method))
             ...
             (wrapped-flag 
              (cond ((or (string= "+createBegin:" signature)
                     ...
                     (insert "SD_JAVA_ADDJAVA (env, jobj, ")
                     t)
                    ((create-method-p method)
                     (insert "SD_JAVA_ADDJAVA (env, nextPhase, ")
                     t)
                    ((or (string= "-createEnd" signature))
                     (insert "SD_JAVA_NEXTPHASE (env, jobj, ")
                     t)
                    ((string= java-return "Class")
                     (insert "(jclass) SD_JAVA_FINDJAVACLASS (env, ")
                     t)
      ...
      (insert ";\n")
      (when strings
        (insert "  SD_JAVA_CLEANUPSTRINGS (env, strings);\n"))
      (when (convenience-create-method-p protocol method)
