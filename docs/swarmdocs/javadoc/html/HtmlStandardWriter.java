/*
 * @(#)HtmlStandardWriter.java	1.28 98/08/18
 *
...
public class HtmlStandardWriter extends HtmlDocWriter {
    ...
    public void printHeader(String title) {
        println("<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.0//EN\">");
        println("<!--NewPage-->");
        html();
        head();
    ...
    public void printUserHeaderFooter(boolean header) {
        em();
        if (header) {
            print(Standard.configuration().header);
        } else {
            if (Standard.configuration().footer.length() != 0) {
                print(Standard.configuration().footer);
            } else {
                print(Standard.configuration().header);
	    }
        }
    ...
    public void printBottom() {
        hr();
        print(Standard.configuration().bottom);  // usually empty
    } 
    ...
    public void printPackageLink(PackageDoc pkg, String linklabel) {
        print(getPackageLink(pkg, linklabel));
    }
    ...
    public String getPackageLink(PackageDoc pkg) {
        return getPackageLink(pkg, pkg.name());
    }
    ...
    public String getPackageLink(PackageDoc pkg, String linklabel) {
        if (pkg.isIncluded()) { 
            ...
            if (crossPkgLink != null) {
                return getHyperLink(crossPkgLink, linklabel);
            } else {
                return linklabel;
            }
        }
    ...
    public void printClassLinkForSameDir(ClassDoc cd) {
        if (cd.isIncluded()) {
            printHyperLink(cd.name() + ".html", "", italicsClassName(cd, 
                                                                     false));
        } else {
   ...
   /**
        ...
        if (refClass == null) {
            if (refPackage != null && refPackage.isIncluded()) { 
                ...
                if (refPackName != null && refPackName.length() > 0) {
                    ...
                    error("doclet.see.malformed_tag", seetext);
                }
            } 
    ...
    public void printSinceTag(Doc doc) {
        Tag[] sinces = doc.tags("since");
        if (sinces.length > 0) {   // there is going to be only one since tag
            dt();
