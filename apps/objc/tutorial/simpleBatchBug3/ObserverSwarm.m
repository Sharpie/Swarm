...
+ createBegin: aZone
{
  ...
  [probeMap addProbe:  [probeLibrary getProbeForMessage: "saveSerialData:"
				     inClass: [self class]]];
...
- buildObjects
{
  ...
  if (  [(Parameters*) arguments getFilename] )  
    {
      char * name = [(Parameters*) arguments getFilename]; 
      
      id archiver;

      fprintf(stderr,"name is %c\n",name[0]);
      archiver = [LispArchiver create: self 
			       setPath: name];
      
      modelSwarm = [archiver getObject: "modelSwarm"];
      [archiver drop];
    }

  // if there is no filename set from the command line
  // use the default "bug.scm".  We could write as in the
  // previous method, but for variety we leave the usage
  // of the default LispArchiver object. It grabs parameters
  // from the scm file that has the same name as the program.
  else
    {
      modelSwarm = 
	[lispAppArchiver getWithZone: self key: "modelSwarm"];
  ...
  if (  [(Parameters*) arguments getFilename] == NULL ) 
    {
...
- takeScreenShot
{
  char filename[40];
  id aPix;
  long unsigned currentTime = [(Parameters*)arguments getCurrentTime];
...
{
  [modelSwarm lispArchive: outname];
  return outname;
}
